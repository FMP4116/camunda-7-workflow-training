...
En el **explorador de archivos** de VS Code, entra en **workflow-app** → **src** → **main** → **java**. Ahí verás los paquetes existentes (por ejemplo `com.example.workflow`). **Clic derecho** en el paquete raíz (por ejemplo `workflow`) → **New Folder** y crea **delegate** (o si tu proyecto usa otra estructura, crea la carpeta que corresponda al paquete `com.example.workflow.delegate`: por ejemplo las carpetas `com/example/workflow/delegate` dentro de **java**).

---

# Crear la clase Java

Dentro de la carpeta **delegate** (o **workflow/delegate** según tu estructura), **clic derecho** → **New File** y crea **ValidarSolicitudDelegate.java**. La ruta debe quedar algo como **workflow-app/src/main/java/com/example/workflow/delegate/ValidarSolicitudDelegate.java** (ajusta `com.example.workflow` al paquete real de tu proyecto).
...
Abre una **terminal**. Desde la **raíz del repositorio** ejecuta `cd workflow-app`. Luego:
